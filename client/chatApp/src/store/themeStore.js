...
export const useThemeStore = create((set, get) => ({
  ...
  // 🔄 Hydrate theme on app load + start cross-tab sync (only for notifications now)
  hydrateTheme: () => {
    const theme = sessionStorage.getItem("chatflow-theme") || "light";
    document.documentElement.classList.toggle("dark", theme === "dark");
    set({ theme });

    // Set up cross-tab sync via storage event (for notifications)
    if (_storageHandler) {
